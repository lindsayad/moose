...
ParsedCurveGenerator::validParams()
{
  ...
  params.addRequiredParam<std::string>("x_formulus", "Function expression of x(t)");
  params.addRequiredParam<std::string>("y_formulus", "Function expression of y(t)");
  params.addParam<std::string>("z_formulus", "0", "Function expression of z(t)");
  params.addRequiredRangeCheckedParam<std::vector<unsigned int>>(
      "nums_segments",
      "nums_segments>=1",
      "Numnbers of segments of segments of the curve to be generated.");
...
ParsedCurveGenerator::ParsedCurveGenerator(const InputParameters & parameters)
  : MeshGenerator(parameters),
    ...
    _function_x(getParam<std::string>("x_formulus")),
    _function_y(getParam<std::string>("y_formulus")),
    _function_z(getParam<std::string>("z_formulus")),
                                           ...
                                           const Real oversample_factor)
{
  ...
  if (MooseUtils::absoluteFuzzyEqual(total_distance, 0.0))
    paramError("t_end", "The curve has at least one cross which is not supported.");
  ...
  while (t_space_id + 1 < t_sect_space.size())
  {
