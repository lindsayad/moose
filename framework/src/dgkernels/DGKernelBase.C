...
DGKernelBase::DGKernelBase(const InputParameters & parameters)
  : MooseObject(parameters),
    ...
    Restartable(parameters, "DGKernelBases"),
...
void
DGKernelBase::computeElemNeighResidual(Moose::DGResidualType type)
{
  bool is_elem;
...
void
DGKernelBase::computeElemNeighJacobian(Moose::DGJacobianType type)
{
...
void
DGKernelBase::computeOffDiagElemNeighJacobian(Moose::DGJacobianType type, unsigned int jvar)
{
  const VariableTestValue & test_space =
      (type == Moose::ElementElement || type == Moose::ElementNeighbor) ? _test : _test_neighbor;
  const VariableTestValue & loc_phi =
...
Real
DGKernelBase::computeQpOffDiagJacobian(Moose::DGJacobianType /*type*/, unsigned int /*jvar*/)
{
  return 0.;
}
