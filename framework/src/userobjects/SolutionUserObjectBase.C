...
SolutionUserObjectBase::validParams()
{
  ...
  params.addParam<MultiMooseEnum>(
      ...
      "form p = R1*(R0*x*m - t)/s.  Then the values provided by the SolutionUserObjectBase at "
      "point x "
...
Threads::spin_mutex SolutionUserObjectBase::_solution_user_object_mutex;
...
void
SolutionUserObjectBase::readXda()
{
...
SolutionUserObjectBase::readExodusIIOrNemesis()
{
  ...
  // Helper for copying from the ExodusII/Nemesis file
  const auto copy_solutions = [this](auto & io, libMesh::System & system, unsigned int index)
  {
    for (const auto & var_name : _nodal_variables)
      io.copy_nodal_solution(system, var_name, var_name, index);

    for (const auto & var_name : _elemental_variables)
      io.copy_elemental_solution(system, var_name, var_name, index);

    if (_scalar_variables.size() > 0)
      io.copy_scalar_solution(system, _scalar_variables, _scalar_variables, index);
  };
  ...
  if (_interpolate_times)
  {
    ...
    updateInterpolationBracketingTimeIndices();
  ...
  else
  {
    if (_exodus_time_index > num_exo_times)
      mooseError("In SolutionUserObjectBase, timestep = ",
                 _exodus_time_index,
                 ", but there are only ",
                 num_exo_times,
                 " time steps.");
...
Real
SolutionUserObjectBase::directValue(const Node * node, const std::string & var_name) const
{
...
Real
SolutionUserObjectBase::directValue(const Elem * elem, const std::string & var_name) const
{
  // Get the libmesh variable and system numbers
  unsigned int var_num = _system->variable_number(var_name);
  unsigned int sys_num = _system->number();
...
void
SolutionUserObjectBase::finalize()
{
}
...
{
  // Update time interpolation for ExodusII or Nemesis solution
  if (_interpolate_times)
    updateTimeInterpolationFromFile();
...
void
SolutionUserObjectBase::execute()
{
}
...
SolutionUserObjectBase::initialSetup()
{
  ...
  // Produce an error for an unknown file type
    ...
    mooseError(
        "In SolutionUserObjectBase, invalid file type: only .xda, .xdr, .exo and .e supported");
...
SolutionUserObjectBase::updateTimeInterpolationFromFile()
{
  ...
  {
    if (updateInterpolationBracketingTimeIndices())
    {
      // Helper to copy/update solutions from a file IO to a system
      const auto update_solutions = [this](auto & io, libMesh::System & system, unsigned int index)
      {
        for (const auto & var_name : _nodal_variables)
          io.copy_nodal_solution(system, var_name, var_name, index);
        for (const auto & var_name : _elemental_variables)
...
bool
SolutionUserObjectBase::updateInterpolationBracketingTimeIndices()
{
  if (_file_type != "exodusII" && _file_type != "nemesis")
  ...
  const auto solution_time = solutionSampleTime();
...
unsigned int
SolutionUserObjectBase::getLocalVarIndex(const std::string & var_name) const
{
...
Real
SolutionUserObjectBase::pointValue(Real libmesh_dbg_var(t),
                                   const Point & p,
                                   ...
                                   const std::set<subdomain_id_type> * subdomain_ids) const
{
  ...
  // Interpolate
  if (_interpolate_times && (_file_type == "exodusII" || _file_type == "nemesis"))
  {
...
RealGradient
SolutionUserObjectBase::pointValueGradient(Real libmesh_dbg_var(t),
                                           Point pt,
                                           const unsigned int local_var_index,
                                           const std::set<subdomain_id_type> * subdomain_ids) const
{
...
SolutionUserObjectBase::discontinuousPointValueGradient(
    Real t,
    const Point & p,
    const std::string & var_name,
    const std::set<subdomain_id_type> * subdomain_ids) const
...
Real
SolutionUserObjectBase::evalMeshFunction(const Point & p,
                                         const unsigned int local_var_index,
...
std::map<const Elem *, Real>
SolutionUserObjectBase::evalMultiValuedMeshFunction(
    const Point & p,
    const unsigned int local_var_index,
    unsigned int func_num,
    const std::set<subdomain_id_type> * subdomain_ids) const
{
  ...
  {
    mooseAssert(
        k.second.size() > local_var_index,
        "In SolutionUserObjectBase::evalMultiValuedMeshFunction variable with local_var_index "
...
RealGradient
SolutionUserObjectBase::evalMeshFunctionGradient(
    const Point & p,
    const unsigned int local_var_index,
    unsigned int func_num,
    const std::set<subdomain_id_type> * subdomain_ids) const
...
std::map<const Elem *, RealGradient>
SolutionUserObjectBase::evalMultiValuedMeshFunctionGradient(
    const Point & p,
    const unsigned int local_var_index,
    unsigned int func_num,
    const std::set<subdomain_id_type> * subdomain_ids) const
...
SolutionUserObjectBase::variableNames() const
{
  return _system_variables;
}

bool
SolutionUserObjectBase::isVariableNodal(const std::string & var_name) const
{
...
Real
SolutionUserObjectBase::scalarValue(Real /*t*/, const std::string & var_name) const
{
  unsigned int var_num = _system->variable_number(var_name);
