...
class Test1DRC():
    def test(self):
        labels = ['L2u', 'L2p']
        df1 = mms.run_spatial('1d-rc.i', list(range(1,9)), "--error", y_pp=labels, mpi=1)
...
class Test2DRC():
    def test(self):
        labels = ['L2u', 'L2v', 'L2p']
        df1 = mms.run_spatial('2d-rc.i', 6, "--error", y_pp=labels, mpi=8)
...
class Test2DRCWalls():
    def test(self):
        labels = ['L2u', 'L2v', 'L2p']
        df1 = mms.run_spatial('2d-rc-no-slip-walls.i', 5, "--error", y_pp=labels, mpi=8)
