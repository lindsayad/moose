...
class Test2DRC(unittest.TestCase):
    def test(self):
        labels = ['L2u', 'L2v', 'L2p']
        df1 = run_spatial('2d-rc.i', 7, y_pp=labels, mpi=16)
...
class Test2DRCTemp(unittest.TestCase):
    def test(self):
        labels = ['L2u', 'L2v', 'L2p', 'L2t']
        df1 = run_spatial('2d-average-with-temp.i', 7, "velocity_interp_method='rc'", "UserObjects/rc/standard_body_forces=false", y_pp=labels, mpi=16)
