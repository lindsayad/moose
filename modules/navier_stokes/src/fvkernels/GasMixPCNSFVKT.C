...
registerMooseObject("NavierStokesApp", GasMixPCNSFVKT);

InputParameters
GasMixPCNSFVKT::validParams()
{
...
GasMixPCNSFVKT::GasMixPCNSFVKT(const InputParameters & params)
  : FVFluxKernel(params),
    ...
    _knp_for_omega(getParam<bool>("knp_for_omega"))
{
  ...
  mooseError(
      "GasMixPCNSFVKT objects require global AD indexing because they  require gradient "
      "reconstruction. Please cd into the root of your MOOSE directory and run './configure'");
#endif
}

std::pair<ADReal, ADReal>
GasMixPCNSFVKT::computeAlphaAndOmega(const ADReal & u_elem_normal,
...
ADReal
GasMixPCNSFVKT::computeFaceFlux(const ADReal & alpha,
                                const ADReal & omega,
...
ADReal
GasMixPCNSFVKT::computeQpResidual()
{
  ...
  const auto fraction_elem = interpolate(*_limiter,
                                         _fraction_elem[_qp],
                                         _fraction_neighbor[_qp],
                                         &_grad_fraction_elem[_qp],
                                         *_face_info,
                                         /*elem_is_up=*/true);
  const auto fraction_neighbor = interpolate(*_limiter,
                                             _fraction_neighbor[_qp],
                                             _fraction_elem[_qp],
                                             &_grad_fraction_neighbor[_qp],
                                             *_face_info,
                                             /*elem_is_up=*/false);

  std::vector<ADReal> mass_fractions_elem(1), mass_fractions_neighbor(1);
  mass_fractions_elem[0] = fraction_elem;
  mass_fractions_neighbor[0] = fraction_neighbor;
  ...
  const auto rho_elem = _fluid.rho_from_p_T_X(pressure_elem, T_fluid_elem, mass_fractions_elem);
  const auto specific_volume_elem = 1. / rho_elem;
  const auto e_elem = _fluid.e_from_p_T_X(pressure_elem, T_fluid_elem, mass_fractions_elem);
  ...
  const auto rho_neighbor =
      _fluid.rho_from_p_T_X(pressure_neighbor, T_fluid_neighbor, mass_fractions_neighbor);
  const auto specific_volume_neighbor = 1. / rho_neighbor;
  const auto e_neighbor =
      _fluid.e_from_p_T_X(pressure_neighbor, T_fluid_neighbor, mass_fractions_neighbor);

  const auto c_elem = _fluid.c_from_v_e_X(specific_volume_elem, e_elem, mass_fractions_elem);
  const auto c_neighbor =
      _fluid.c_from_v_e_X(specific_volume_neighbor, e_neighbor, mass_fractions_neighbor);
