...
{
  // A lot of RC data gathering leverages the automatic differentiation system, e.g. for linear operators we pull out the 'a' coefficents by querying the ADReal residual derivatives member  at the element or neighbor dof locations. Consequently we need to enable derivative computation. We do this here outside the threaded regions
