...
INSFVVelocityVariable::adGradSln(const Elem * const elem) const
{
  ...
  // $\hat{e_b}$ replaced with $\hat{n}$ because we believe the equation as written doesn't reflect
  // the intent of the text, which is to guarantee a zero normal gradient in the direction of the
  // surface normal
  ...
  // ebf eqns: element gradient coefficients, e.g. eqn. 2, LHS term 2 coefficient. *Note* that each
  // element of the std::vector could correspond to a cell centroid gradient or to a face gradient
  // computed on a fully developed flow face
